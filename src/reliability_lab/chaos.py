    ...
    for _ in range(config.load_test.requests):
        ...
        if result.cache_hit:
            ...
            metrics.estimated_cost_saved += (
                avoided_tokens / 1000.0 * primary.cost_per_1k_tokens
            )
...
def run_simulation(config: LabConfig, queries: list[str]) -> RunMetrics:
    ...
    Cache/no-cache and concurrent comparisons are produced by
    ``scripts/run_benchmarks.py`` so the chaos aggregate remains focused.
    """
    random.seed(42)
    ...
    for scenario in config.scenarios:
        ...
        if scenario.name == "primary_timeout_100":
            ...
            passed = result.availability >= 0.95 and result.fallback_successes > 0
        elif scenario.name == "all_healthy":
            passed = result.availability >= 0.99
        elif scenario.name == "all_providers_down":
            passed = (
                result.failed_requests == result.total_requests
                and result.circuit_open_count >= len(config.providers)
            )
        else:
            passed = result.availability >= 0.95
        combined.scenarios[scenario.name] = "pass" if passed else "fail"
        result_report = result.to_report_dict()
        result_report.pop("scenarios")
        result_report.pop("scenario_results")
        combined.scenario_results[scenario.name] = result_report
